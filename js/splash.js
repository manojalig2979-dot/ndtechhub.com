...
(function () {
  ...
  function initSplashScreen() {
    ...
    if (!splashEl) return;
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || window.matchMedia('(max-width: 700px)').matches) {
      splashEl.classList.add('reduced-motion');
    }
    ...
    const lightweightDevice = window.matchMedia('(prefers-reduced-motion: reduce)').matches || window.matchMedia('(max-width: 700px)').matches;
    if (typeof THREE !== 'undefined' && !lightweightDevice) {
      initThreeScene();
    } else {
  ...
  function animateThree() {
    ...
    if (document.hidden) { animationFrameId = requestAnimationFrame(animateThree); return; }
